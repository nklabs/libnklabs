...
int ZmodemTFile(const char	*file_name,
                ...
                int	bytesRem,
                ZModem	*info ) {
	if( file_name == NULL || (info->file = jzopen(file_name, "rb")) == NULL ) {
...
*/
	...
	info->len = jzsize(info->file);
	info->mode = (0666|0100000);
	}

	if( info->Protocol == ZModem_XMODEM )
		return YSendData(info);

	if( info->Protocol == ZModem_YMODEM ) {
...
int sendFilename( ZModem *info ) {
	...
	ptr += i+1;
	nk_snprintf( (char*)( ptr ) , 125, "%d %lo %o 0 %d %d 0", info->len, info->date,
	...
	*/
	...
	if( (info->zrinitflags & (CANFDX|CANOVIO)) == (CANFDX|CANOVIO) &&
		...
		if( info->windowsize == 0 ) {
			info->Streaming = ZModem_Full;
		} else {
			info->Streaming = ZModem_StrWindow;
		}
	}

	else if( (info->zrinitflags & (CANFDX|CANOVIO)) == (CANFDX|CANOVIO) &&
	         info->bufsize == 0 ) {
		info->Streaming = ZModem_SlidingWindow;
	}

	else {
		info->Streaming = ZModem_Segmented;
	}
...
int startFileData( ZModem *info ) {
	...
	jzseek(info->file, info->offset, 0);
...
int SendMoreFileData( ZModem *info ) {
	...
	*/
	...
	if( info->waitflag ) {
		...
		case ZModem_Full:
		case ZModem_Segmented:
			type = ZCRCG;
			break;

		case ZModem_StrWindow:
	...
	*/
	...
	while( len > 0  && (c = jzgetc(info->file)) != EOF ) {
		if( !crc32 ) {
			crc = updcrc(c, crc);
		} else {
			crc = UPDC32(c, crc);
	...
	*ptr++ = type;
	...
	if( !crc32 ) {
		crc = updcrc(0,crc);
		crc = updcrc(0,crc);
		ptr = putZdle( ptr, (u_char)( ( crc >> 8 ) & 0xff ), info );
		ptr = putZdle( ptr, (u_char)( crc & 0xff ), info);
	} else {
		crc = ~crc;
...
int YsendChar( char c, ZModem *info ) {
	...
	case YTEOF:		/* sent EOF, waiting for ACK or NAK */
		...
		case ACK:
			info->state = info->Protocol == ZModem_YMODEM ? YTStart : Done;
			return ZmDone;
		default:
			return 0;
		}
...
int YSendFilename( ZModem *info ) {
	...
	memcpy(ptr, info->rfilename, i+1);
	ptr += i+1;
	nk_snprintf( (char*)( ptr ), 125, "%d %lo %o 0", info->len, info->date, info->mode);
	ptr += strlen( (char*)( ptr ) );
	*ptr++ = '\0';
...
int YSendData( ZModem *info ) {
	...
	if( info->chrCount <= 0 ) {
		info->bufp = 0;
		info->chrCount = jzread(info->buffer, 1, info->packetsize, info->file);
		info->fileEof = jzeof(info->file);
	}
