...
#define	MaxNoise	64	/* max "noise" characters before transmission
				 * pauses */
#define	MaxErrs		40	/* Max receive errors before cancel */
...
enum  {ZModem_Idle, ZModem_Padding, ZModem_Inhdr, ZModem_Indata, ZModem_Finish, ZModem_Ysend, ZModem_Yrcv};
enum  {ZModem_XMODEM, ZModem_YMODEM, ZModem_ZMODEM};
enum {ZModem_Full, ZModem_StrWindow, ZModem_SlidingWindow, ZModem_Segmented};
...
struct ZModem {
	...
	int	jinterrupt ;	/* received attention signal */
...
FILE *jzopen(const char *name, const char *mode);
void jzclose(FILE *f);
int jzread(unsigned char *buffer, int ign, int size, FILE *f);
int jzeof(FILE *f);
void jzseek(FILE *f, size_t offset, int ign);
int jzsize(FILE *f);
int jzgetc(FILE *f);
